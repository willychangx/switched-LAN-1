...
while(1) {
	...
	if (n>0) {
		switch(man_cmd) {
			...
			case 'd': /* Download file from host */
				sscanf(man_msg, "%d %s %d", orig, name, &dst);
				new_job = (struct host_job *)
					malloc(sizeof(struct host_job));
				new_job->type = JOB_FILE_DOWNLOAD_SEND;
				new_job->file_upload_dst = orig;
				for(i=0; name[i] != '\0'; i++){
		...
		case JOB_FILE_UPLOAD_SEND:
			...
			if (dir_valid == 1) {
				...
				if (fp != NULL) {
					 ...
					 */
					n = fread(string,sizeof(char), 1000, fp);
					fclose(fp);
					string[n] = '\0';
